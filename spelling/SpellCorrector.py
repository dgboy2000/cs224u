...
class SpellCorrector:
        ...
        for row in reader:
            ...
            if count > 0:
                ...
                if bigram in SpellCorrector.bigrams:
                    ...
                    SpellCorrector.bigrams[bigram] = count
                total_count += count
               #if row[1] in SpellCorrector.dictionary and row[2] in SpellCorrector.dictionary:
                   #SpellCorrector.bigrams[row[1] + ' ' + row[2]] = count
    ...
    def getScore(self, word, previous, option, next):
        ...
        edit_distance = self.getEditDistance(word, option)
    ...
    def getEditDistance(self, a, b):
        if len(a) is 0 and len(b) is 0:
            ...
            return len(a)
        if a[0] is b[0]:
            return self.getEditDistance(a[1:len(a)], b[1:len(b)])
        insert_dist = self.getEditDistance(a[1:len(a)], b)
        delete_dist = self.getEditDistance(a, b[1:len(b)])
        sub_dist = self.getEditDistance(a[1:len(a)], b[1:len(b)])
